...
const updateFormValues = (data: Record<string, any>): void => {
  if (props.routeId) {
    recordFlavor.value = typeof data.expression === 'string' && data.expression.length > 0 ? RouterFlavor.EXPRESSIONS : RouterFlavor.TRADITIONAL
    currentConfigTab.value = recordFlavor.value
  }
